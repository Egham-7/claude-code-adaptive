...
const transformClaudeToOpenAI = (
	...
	const { messages, system, tools, model, max_tokens, temperature, stream, tool_choice } =
		claudeRequest;
			...
			if (typeof msg.content === "string") {
				...
				if (msg.role === "user") {
					...
					const textAndMediaParts = (msg.content || []).filter(
						(c: any) => c && ((c.type === "text" && c.text) || (c.type === "image" && c.source))
					);
					...
					if (textParts.length > 0) {
						message.content = textParts.map((text: any) => text.text).join("\n");
					}
...
const transformOpenAIToClaude = (openAIResponse: ChatCompletion): Message => {
	...
	return {
		...
		stop_reason: (stopReasonMapping[choice.finish_reason as keyof typeof stopReasonMapping] ||
			"end_turn") as "end_turn" | "max_tokens" | "tool_use" | "stop_sequence",
		stop_sequence: null,
		usage: usage
			? {
...
const forwardToOpenAI = async (req: any, reply: any, config: RouterConfig) => {
	try {
		...
		log("Sending request to OpenAI API:", JSON.stringify(openAIRequest, null, 2));
		...
		if (openAIRequest.stream) {
			...
			log("Transformed Claude response:", JSON.stringify(claudeResponse, null, 2));
